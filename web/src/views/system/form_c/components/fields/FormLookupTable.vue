...
import { FormLookupItemForm } from '.';
import { GroupActions } from '../../../lookup/components';
import GenericFormModal from '../GenericFormModal.vue';
...
// Reactive state
const formLookupItemFormRef = ref<any>(null);
...
async function fetchPage(p = 1) {
  ...
  try {
    const res = await formLookupService.listItems(effectiveGroupId(), {
      page: p,
...
function openAdd() {
  if (props.addForm) {
    ...
    formLookupItemFormRef.value?.open({
      initial: null,
...
async function handleCreateOrUpdate(payload: any) {
  ...
  if (payload.id) {
    ...
    try {
      const res = await formLookupService.updateItem(
    ...
    try {
      const res = await formLookupService.createItem(
        effectiveGroupId() || 'unknown',
        itemPayload,
      );
...
async function removeBulk(ids: string[]) {
  ...
  try {
    await formLookupService.deleteItems(effectiveGroupId() || 'unknown', ids);
