...
* **`fieldId` (String):** The `id` of the *other* field this one depends on (e.g., "G7a").
...
The `FormSubmission` document is a **record** that stores one user's (e.g., one company's) data for a *specific* `FormTemplate`. It is a simple key-value store of the answers.
